...
check("3.8", "sitemap count", (fs.readFileSync("out/sitemap.xml", "utf8").match(/<url>/g) || []).length === 13);
...
check("4.4", "lang=en", /lang="en"/.test(home));
...
for (const tag of home.match(/src="(\/_next\/static\/chunks\/[^"]+\.js)"/g) || []) {
  ...
  if (size > 204800) {
    over200++;
    console.log(`  initial over200KB: ${chunkPath} (${size})`);
  }
}
check("6.4", `initial load JS (${over200} over 200KB)`, over200 === 0);
